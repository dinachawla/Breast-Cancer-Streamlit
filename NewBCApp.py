...
# Create an explicit malignant indicator (1 = malignant, 0 = benign)
...
with right_col:
    with st.container(key="sticky-right-column"):
        ...
        for feature, user_val in values.items():
            ...
            min_val, max_val = user_val - margin, user_val + margin
            nearby_cases = df[(df[feature] >= min_val) & (df[feature] <= max_val)]
            malignant_pct = (
                100 * nearby_cases["is_malignant"].mean()
        ...
        st.caption(
            "Model is for educational use only and **does not replace professional medical advice.**"
        )
